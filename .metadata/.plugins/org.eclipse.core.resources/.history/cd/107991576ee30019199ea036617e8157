...
import com.gmail.stefan.ui.encoders.LocalDateToStringEncoder;
import com.gmail.stefan.ui.encoders.LongToStringEncoder;
import com.gmail.stefan.ui.views.reviewslist.ReviewsList.ReviewsModel;
...
public class ReviewsList extends PolymerTemplate<ReviewsModel> {
    ...
    public interface ReviewsModel extends TemplateModel {
        ...
        @Encode(value = LongToStringEncoder.class, path = "taster.id")
    	@Encode(value = LongToStringEncoder.class, path = "beverage.id")
    ...
    private ReviewEditorDialog reviewForm = 
    		new ReviewEditorDialog(this::saveUpdate, this::deleteUpdate);

    public ReviewsList() {																// @'HtmlImport' dependency filter?
        search.setPlaceholder("Search reviews");
        search.addValueChangeListener(e -> updateList());
        search.setValueChangeMode(ValueChangeMode.EAGER);
        search.addFocusShortcut(Key.KEY_F, KeyModifier.CONTROL);

        addReview.addClickListener(e -> openForm(new Review(),							// new review
